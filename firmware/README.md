...
- **Enrich flights** with readable airline/aircraft info from AeroAPI and the FlightWatch CDN.
...
- **adapters/FlightWallFetcher**: Looks up human-friendly airline/aircraft names from the FlightWatch CDN.
...
- Background fetch task (FreeRTOS) every `FETCH_INTERVAL_SECONDS`: OpenSky `states/all` (OAuth) -> AeroAPI enrichment -> FlightWatch name lookups -> `g_lastFlights` (mutex-protected).
...
- Network: outbound HTTPS to OpenSky OAuth + `states/all`, AeroAPI, and FlightWatch CDN; local captive portal for first-time WiFi.
